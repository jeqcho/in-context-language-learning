...
def unigram_batch(batch: t.tensor, num_states, device):
    return t.stack([unigram_strategy(seq_idx, num_states, device) for seq_idx in batch]).to(device)
...
def bigram_batch(batch: t.tensor, num_states, device):
    return t.stack([bigram_strategy(seq_idx, num_states, device) for seq_idx in batch]).to(device)
