...
__all__ = ["SpeedMonitor", "LRMonitor", "Inferencer"]
...
class Inferencer:
    ...
    def get_logits(self, batch: BatchEncoding):
        # Zero gradients and set model to 'eval' mode.
        self.optim.zero_grad(set_to_none=True)
        self.dist_model.eval()

        assert batch.shape[0] == 1, f"batch.shape[0]=1 is only supported now, but got {batch.shape[0]}"

        # Move tensors to the right device.
        batch = move_to_device(batch, self.device)

        # Initialize the output_ids with the input_ids
        output_ids = batch["input_ids"].clone()
        attention_mask = batch["attention_mask"].clone()

        output_ids = move_to_device(output_ids, self.device)
        ...
        with torch.no_grad():  # NOTE: 'torch.inference_mode()' doesn't work with 'torch.compile()'.
            with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                # run inference using model_forward
                _, _, logits = self.model_forward(
                    {"input_ids": output_ids, "attention_mask": attention_mask}, loss_reduction="none"
                )
                return logits

    def generate(self, batch: BatchEncoding, do_sample=True, top_k=5, max_new_tokens=10) -> List[str]:
        # Zero gradients and set model to 'eval' mode.
        self.optim.zero_grad(set_to_none=True)
        self.dist_model.eval()

        assert batch["input_ids"].shape[0] == 1, f"input_ids.shape[0]=1 is only supported now, but got {batch['input_ids'].shape[0]}"
        ...
        for _ in range(max_new_tokens):
            with torch.no_grad():  # NOTE: 'torch.inference_mode()' doesn't work with 'torch.compile()'.
                with torch.autocast("cuda", enabled=True, dtype=self.cfg.autocast_precision):
                    ...
                    if output_ids.shape[1] > self.cfg.model.max_sequence_length:
                        model_input_ids = output_ids[:, -self.cfg.model.max_sequence_length :]
                        model_attention_mask = attention_mask[:, -self.cfg.model.max_sequence_length :]
                    else:
                        model_input_ids = output_ids
                        model_attention_mask = attention_mask
                    _, _, logits = self.model_forward(
                        {"input_ids": model_input_ids, "attention_mask": model_attention_mask}, loss_reduction="none"
                    )
                    # sample the logits using top k
                    # look at the last logit
                    logits = logits[:, -1, :]
                    if do_sample:
                        raise NotImplementedError("sampling is not implemented yet")
                        next_token_ids = torch.topk(logits, top_k, dim=1).indices
                    else:
                        next_token_ids = logits.argmax(dim=-1)
                    # Append the next token to each sequence in the batch
                    print("next_token_ids", next_token_ids)
                    output_ids = torch.cat([output_ids, next_token_ids.unsqueeze(1)], dim=-1)

                    # Update the attention mask for the new tokens
                    new_attention_mask = torch.ones(next_token_ids.shape, device=self.device)
                    attention_mask = torch.cat([attention_mask, new_attention_mask.unsqueeze(1)], dim=-1)
